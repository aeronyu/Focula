...
public final class DatabaseStore {
    ...
    public static func applicationStoreURL() throws -> URL {
        ...
        return try applicationStoreURL(base: base)
    }

    public static func applicationStoreURL(base: URL, fileManager: FileManager = .default) throws -> URL {
        let folder = base.appendingPathComponent("Focula", isDirectory: true)
        let legacyFolder = base.appendingPathComponent("Watch My Back", isDirectory: true)
        if !fileManager.fileExists(atPath: folder.path),
           fileManager.fileExists(atPath: legacyFolder.path) {
            try fileManager.moveItem(at: legacyFolder, to: folder)
        }
        try fileManager.createDirectory(at: folder, withIntermediateDirectories: true)
        let storeURL = folder.appendingPathComponent("focula.sqlite")
        let legacyStoreURL = folder.appendingPathComponent("watch-my-back.sqlite")
