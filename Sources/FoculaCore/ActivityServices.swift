...
public struct FrontmostAppSnapshot: Equatable, Sendable {
    ...
    public var visibleApps: [VisibleAppSnapshot]

    public init(
        appName: String,
        bundleIdentifier: String?,
        windowFrame: CGRect? = nil,
        visibleApps: [VisibleAppSnapshot] = []
    ) {
...
public struct VisibleAppSnapshot: Equatable, Sendable {
    public var appName: String
    public var bundleIdentifier: String?
    public var windowCount: Int
    public var isFrontmost: Bool

    public init(appName: String, bundleIdentifier: String?, windowCount: Int, isFrontmost: Bool) {
        self.appName = appName
        self.bundleIdentifier = bundleIdentifier
...
public enum ActivityContextText {
    public static func make(
        focusedApp: FrontmostAppSnapshot,
        recentSummaries: [String] = []
    ) -> [String] {
        var lines = [
            "Focused app: \(focusedApp.appName)\(focusedApp.bundleIdentifier.map { " (\($0))" } ?? "")."
        ]
        let visibleApps = focusedApp.visibleApps.isEmpty
            ? [VisibleAppSnapshot(
                appName: focusedApp.appName,
                bundleIdentifier: focusedApp.bundleIdentifier,
                windowCount: focusedApp.windowFrame == nil ? 0 : 1,
                isFrontmost: true
            )]
            : focusedApp.visibleApps

        let visibleSummary = visibleApps
            .prefix(12)
            .map { app in
                let role = app.isFrontmost ? "focused" : "visible"
                let count = app.windowCount == 1 ? "1 window" : "\(app.windowCount) windows"
                return "\(app.appName) (\(role), \(count))"
            }
            .joined(separator: "; ")
        lines.append("Visible apps on active displays: \(visibleSummary).")

        if !recentSummaries.isEmpty {
            lines.append("Recent activity summaries: \(recentSummaries.joined(separator: " | "))")
        }
        return lines
    }
}
...
public struct NSWorkspaceFrontmostAppProvider: FrontmostAppProviding {
    ...
    public func currentApp() -> FrontmostAppSnapshot {
        ...
        return FrontmostAppSnapshot(
            ...
            visibleApps: Self.visibleApps(frontmostProcessIdentifier: app?.processIdentifier)
        )
    }
    ...
    private static func visibleApps(frontmostProcessIdentifier: pid_t?) -> [VisibleAppSnapshot] {
        ...
        var grouped: [pid_t: (appName: String, bundleIdentifier: String?, count: Int, isFrontmost: Bool)] = [:]
        ...
        for window in windows {
            ...
            let isFrontmost = processIdentifier == frontmostProcessIdentifier
            var current = grouped[processIdentifier] ?? (appName, bundleIdentifier, 0, isFrontmost)
            current.count += 1
            current.isFrontmost = current.isFrontmost || isFrontmost
            grouped[processIdentifier] = current
        }
        ...
        return grouped.values
            ...
            .sorted {
                ...
                if $0.windowCount != $1.windowCount { return $0.windowCount > $1.windowCount }
