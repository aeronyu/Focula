...
final class AppModel: ObservableObject {
    ...
    @Published private(set) var lastFocusState: FoculaCore.FocusState = .unknown
    ...
    private func classifyCurrentScreen(goal: Goal, app: FrontmostAppSnapshot) async -> VisionClassifierResult {
        do {
            ...
            let contextImageData = settings.modelSelection.provider == .builtInGemma
                ? []
                : capture.contextImageData + contextImageDataForCurrentFrame()
            let contextSummaryText = ActivityContextText.make(
                focusedApp: app,
                recentSummaries: contextSummaryTextForCurrentFrame()
            )
            ...
            rememberContextFrame(imageData, result: result, app: app)
            return result
    ...
    private func rememberContextFrame(_ imageData: Data, result: VisionClassifierResult, app: FrontmostAppSnapshot) {
        screenshotContextFrames.append(ScreenshotContextFrame(
            timestamp: Date(),
            appName: app.appName,
            bundleIdentifier: app.bundleIdentifier,
            activitySummary: ActivitySummaryRedactor.redact(result.activitySummary),
            imageData: imageData
        ))
        pruneContextFrames()
    }
    ...
    private func contextSummaryTextForCurrentFrame() -> [String] {
        pruneContextFrames()
        return screenshotContextFrames
            .suffix(5)
            .compactMap { frame in
                guard let summary = frame.activitySummary, !summary.isEmpty else { return nil }
                return "\(frame.appName): \(summary)"
            }
    }
