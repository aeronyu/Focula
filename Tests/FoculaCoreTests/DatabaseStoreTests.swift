...
final class DatabaseStoreTests: XCTestCase {
    func testApplicationStoreURLMigratesLegacyFolderAndDatabaseName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("focula-db-support-\(UUID().uuidString)", isDirectory: true)
        let support = root.appendingPathComponent("Application Support", isDirectory: true)
        let legacyFolder = support.appendingPathComponent("Watch My Back", isDirectory: true)
        let legacyStore = legacyFolder.appendingPathComponent("watch-my-back.sqlite")
        try FileManager.default.createDirectory(at: legacyFolder, withIntermediateDirectories: true)
        try Data("legacy db".utf8).write(to: legacyStore)
        defer { try? FileManager.default.removeItem(at: root) }

        let storeURL = try DatabaseStore.applicationStoreURL(base: support)

        XCTAssertEqual(storeURL.path, support.appendingPathComponent("Focula/focula.sqlite").path)
    ...
    func testPersistsGoalsAndActivityWithoutScreenshotBytes() throws {
        let path = FileManager.default.temporaryDirectory
            .appendingPathComponent("focula-\(UUID().uuidString).sqlite")
            .path
    ...
    func testMigratesExistingActivityTableForOptionalSummary() throws {
        let path = FileManager.default.temporaryDirectory
            .appendingPathComponent("focula-legacy-\(UUID().uuidString).sqlite")
            .path
    ...
    func testDeletesGoalWithoutDeletingActivityHistory() throws {
        let path = FileManager.default.temporaryDirectory
            .appendingPathComponent("focula-delete-\(UUID().uuidString).sqlite")
            .path
        let store = try DatabaseStore(path: path)
        let goal = Goal.starter
        let sample = ActivitySample(
    ...
    func testPrunesOldActivitySamplesWhileKeepingRecentSamplesPerGoal() throws {
        let path = FileManager.default.temporaryDirectory
            .appendingPathComponent("focula-prune-\(UUID().uuidString).sqlite")
            .path
        let store = try DatabaseStore(path: path)
    ...
    func testPruneKeepsNewerSamplesEvenAfterRecentQuota() throws {
        let path = FileManager.default.temporaryDirectory
            .appendingPathComponent("focula-prune-new-\(UUID().uuidString).sqlite")
            .path
        let store = try DatabaseStore(path: path)
        let goal = Goal.starter
        try store.saveGoal(goal)
