...
final class ActivityServicesTests: XCTestCase {
    func testActivityContextTextIncludesFocusedAndVisibleApps() {
        let focused = FrontmostAppSnapshot(
            appName: "Codex",
            ...
            visibleApps: [
                ...
                VisibleAppSnapshot(
                    appName: "Focula",
                    bundleIdentifier: "dev.aeronyu.Focula",
                    windowCount: 1,
                    isFrontmost: false
                ),
                VisibleAppSnapshot(
        ...
        let context = ActivityContextText.make(
            focusedApp: focused,
            recentSummaries: ["Focula: reviewing activity logs"]
        )

        XCTAssertTrue(context.contains("Focused app: Codex (com.openai.codex)."))
        XCTAssertTrue(context.contains("Visible apps on active displays: Codex (focused, 2 windows); Focula (visible, 1 window); Reminders (visible, 1 window)."))
        XCTAssertTrue(context.contains("Recent activity summaries: Focula: reviewing activity logs"))
    }
